...
\end{itemize}
We then define the function the senstivty function \cite{asd} $S_s^{(t)} = \frac{R_{s;sub}^{(t)}(Q;x) - R_s^{(t)}(Q;x)}{R_{s,sub}^{(t)}(Q;x)-R_s^{(t)}(Q;x)}$. Where $Q\in [Q_{min}, Q_{max}]$ \cite{main}. So now we can define the 
$$
\mathrm{TSF}(x)
= 
\sum_{t \in \mathcal{T}}
