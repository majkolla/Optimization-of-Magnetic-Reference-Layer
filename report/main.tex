\documentclass{article}
\usepackage{graphicx} % Required for inserting images
\usepackage{amsmath}
\usepackage{hyperref}
\usepackage[edges]{forest}

\title{Optimization of Magnetic Reference Layer Design in Polarized Neutron Reflectometry}
\author{Michael Birtman}
\date{October 2025}
 
\begin{document}

\maketitle


\section*{Background}
\paragraph{ALL INFO CURRENTLLY FROM COMPANY PAPER: }
\noindent
\newline
Polarized neutron reflectometry measures how neutrons reflect from a layered sample and provides two reflectivity curves (spin up and spin down). So each neutron spin state has a different SLD, the difference means that by introducing an MRL we get a difference in the reflectivity between different neutron spin channels. The reflectivity is governed by SLD variations at interfaces (WHY?). 
\subsection*{Parratt's Recurisve Method}
FIrst we treat the multilayer as a medium on top of an infientely shick substrate. Now we take $\Delta_j$ as the thickness of each layet and 
$n = 1 - \delta_j + i\beta_j$ as the index of the refraction of each layer. Because it's continuos we know that $k_{xj} = k_x$ and 
we can compute the z-component of $\bar{k}_j$ as $k_{zj}^2 (n_jk)^2 - k_x ^2$.etx etc %ill write it properly later
the intersting result is that we have that for layer $j$ to $j+1$ we get $\prime{r}_{j,j+1} = \frac{Q_j - Q_{j+1}}{Q_j + Q_{j+1}}$ 
We can now compute the reflectivity from the bottom to N:th layer. 






\subsection*{Physics}
\subsubsection*{Kinematics}

\[
Q = \frac{4\pi}{\lambda} \sin\theta.
\]

For layer \( j = 0, \ldots, N \) (\(0\) = ambient, \(N\) = semi-infinite substrate), 
define the \textbf{spin-dependent SLD} in the layer j as: 

\[
\rho^{(\uparrow)}_j(z) = \rho_{n,j}(z) + \rho_{m,j}(z),
\qquad
\rho^{(\downarrow)}_j(z) = \rho_{n,j}(z) - \rho_{m,j}(z).
\]

Here
\[
\rho_n = \sum_i N_i b_i, 
\qquad
\rho_m = C M
\]
(where \(M\) is the in-plane magnetization).  
These are exactly the spin-SLD relations used in the paper.

\medskip

Define the \(z\)-component wavevector in layer \(j\): 

\[
k_{j,s}(Q)
=
\sqrt{\frac{Q^2}{4} - 4\pi \rho^{(s)}_j},
\qquad
s \in \{\uparrow,\downarrow\}.
\]

\subsection*{Interface Fresnel coefficient with roughness (Nevot--Croce)}

\[
r_{j,j+1,s}(Q)
=
\frac{k_{j,s} - k_{j+1,s}}{k_{j,s} + k_{j+1,s}}
\exp\!\left(
-2 k_{j,s} k_{j+1,s} \, \sigma_{j,j+1}^2
\right),
\]

where \(\sigma_{j,j+1}\) is the rms roughness of interface \( j|j+1 \).  
The paper states roughness is included as Gaussian damping at interfaces.

\subsection*{Parratt recursion (propagate phases through thickness \(d_j\))}

For the semi-infinite substrate:

\[
\Gamma_{N,s}(Q) = 0.
\]

For layer \(j\):

\[
\Gamma_{j,s}(Q)
=
\frac{
r_{j,j+1,s}(Q)
+
\Gamma_{j+1,s}(Q)
\exp\!\left(2 i k_{j+1,s}(Q) d_{j+1}\right)
}{
1
+
r_{j,j+1,s}(Q)\,
\Gamma_{j+1,s}(Q)
\exp\!\left(2 i k_{j+1,s}(Q) d_{j+1}\right)
}.
\]

\subsection*{Specular reflectivity}

With optional footprint \(F(Q) \le 1\) and background \(b\):

\[
R_s(Q; x)
=
F(Q)\, \left| \Gamma_{0,s}(Q) \right|^2 + b.
\]




\section*{Baseline Solution}
Since I will experiment with solutions, I think the first solution to gain insight on the problem is by implementing a simple "baseline" solution. This will be done by simply finding the optimum for the strucuture given at the paper [bla bla bla]. This would be so that we can see if the further experimentation on the structural approach is known to better or worse than the baseline solution that is presented in the paper. In part we'll simply solve for the optimal without looking at different materials. 
\newline
\paragraph{Basic}
Let $x = (x_{CoTi}, d_{MRL}, c_{material})$, where 
\begin{itemize}
    \item $x_{CoTi}$ is the alloy composition 
    \item $d_{MRL}$ is the MRL layer thickness 
    \item $c_{mateiral}$ is the capping layer material
\end{itemize}
We want to solve with respect to some metric, using the propsed metric in the paper we get: 

${\rm TSF}(x_{\text{CoTi}},\; d_{\text{MRL}},\; c)$
$x_{\text{CoTi}}\in [0,1]$, $d_{\text{MRL}} > 0$, $c\in{\text{Al}_2\text{O}_3,\;
\text{SiO}_2,\text{Au}}$. 

\paragraph{Basic 2}
We can further introduce more parameters and optimize them. Some of these were static in the paper. So we can do better. Let's define an extended version of $x = (x_{Co}, d_{MRL}, \sigma_{MRL/sub}, \sigma_{MRL/cap}, d_{cap}, \sigma_{cap}, B, \lambda, b)$. 

So we can define a an objective function as: given some SOI $t \in T$. Let's first define the functions that build the sensitivty function. 
\begin{itemize}
    \item $R_s^{(t)}(Q;x)$ simulated polarized refdlectivity of the full stack 
    \item $R_{s, sub}(Q;x)$ same stack without the SOI s
\end{itemize}
We then define the function the senstivty function $S_s^{(t)} = \frac{R_{s;sub}^{(t)}(Q;x) - R_s^{(t)}(Q;x)}{R_{s,sub}^{(t)}(Q;x)-R_s^{(t)}(Q;x)}$. Where $Q\in [Q_{min}, Q_{max}]$ \cite{main}. So now we can define the 
$$
\mathrm{TSF}(x)
= 
\sum_{t \in \mathcal{T}}
\int_{Q_{\min}}^{Q_{\max}}
w(Q)\,
\Big(
\big| S^{(t)}_{\uparrow}(Q; x) \big|
+
\big| S^{(t)}_{\downarrow}(Q; x) \big|
+
\big| S^{(t)}_{\uparrow}(Q; x) - S^{(t)}_{\downarrow}(Q; x) \big|
\Big)
\, dQ.$$ I have added an optinal weight function $w(Q) \equiv1$, but we can define a discrete function or something else so we can prioritize some parts of Q over other parts. We can also break the function down into it's parts getting two functions the ones that together build $\rm TSF$ the point would be to have control over the prioriy of the optimization and build a Pareto optimization and then analyze the priority of the problem. We would do that by simply looking at this. Then we could weight the differnt function such that the solution prioritizes functions differently. From there we could perhaps gain insight to the optimzation problem. 
$$
\max_{x} \big( \mathrm{SFM}_{\mathrm{tot}}(x),\; \mathrm{MCF}(x) \big)
$$





\subsection*{Optimzation method}
To correctly choose the optimzation method we're going to do, we need to understand the landscape of the different functions as well as the computational load each computation of TSF is. This will give insight in what method of optimzation we ought to choose. 

\section*{Defining the Optimization Problem} 
\paragraph{SOURCES: MAINLY MATH BOOKS ON VAR CALC AND OWN REASONIG: }

\noindent
\newline
There are multiple levels of optimization we can apply to this problem. For example, simply constructing an objective function given by \href{} and some restraints. However, we can do better. Firstly there are multiple \emph{objective functions} to look at, some of which are going in different directions. We can also mix different materials, first part will be going through the theory of variational calculus, where we essentially optimizing a function that represents the structure of the thin film. We can do this by applying calculus of variation theory. Here we'd define a functional optimization problem where we can optimize the structure of the thin film. 

We will also be optimizing for the parameters of the structure, let's say the width, the material, the mixture of the material, the cost etc. We will also provide some Pareto optimization techniques such that trade offs of different solutions can be visualized by the experts. Different robustness techniques will be used to test the different peaks and see their resilience to errors in construction. 

In this section we will look at the problem in detail, and derive and create a optimization problem given the problem structure. As the problem heavily depends on the physics of the problem we might be able to look into different fundamental structures of the thin film to improve the results. In this case we need to build a working optimization function that will be able to find stable structure that produce good results given the problem. 


\paragraph{Performance}
We have to define some metric function and we can do that by defining a spin contrast $C(Q; x(z))$, simply a function that measure spin reflectivity moment transfer. We can be creative when defining this function by looking at the difference, log difference, normalize it etc. Currently I'm not at all sure about how we ought to define this function. 

A basic definition could be: 
\[
J_{performance}[x(z)] = \int_{Q_{min}}^{Q_{max}} C(Q;x(z))w(Q)dQ
\]
where $w(Q)$ is simply a weight function to prioritize some Q values, the relevance of this depends on the physics and perhaps this SOI. Currently I'm not sure about how useful this part is. er kan man bara ta vem som helst 

We can also define the \emph{simplest choice} where we just have $C(Q; x(z)) = |R_{\uparrow}(Q) - R_{\downarrow}(Q)|$. This is also of course a non linear optimization problem and defining a Lagrangian to optimize instead may be useful given the structure. Observe the current structure: 

\[
\mathcal{F}[x(z)] = J_{performance} - \lambda_s \int_0 ^L x'(z)^2dz - \lambda_c \int_0 ^LV(x(z))dz - \lambda_r \int_{Q_{min}}^{Q_{max}}D(R(Q;x(z), Q)dQ, \ \lambda_i > 0 
\]

Where, we can define a reflectivity distortion function. There are many ways to define this functional, and depending on what exactly is relevant we may want to change it. 


\section*{Software design}
The design of the package can be viewed as a product of six layers. This is to modularize as much as possible, thus keeping the main parts independent of each other. 

\subsection*{Layer 0}
\begin{description}
  \item[Layer 0 -- Data \& Configs]
    \begin{description}
      \item[data/base1\_materials.json]
        Holds parameters:
        \begin{itemize}
          \item \texttt{substrate.rho\_n}, \texttt{substrate.sigma}
          \item \texttt{caps[name].rho\_n}, \texttt{caps[name].thickness}, \texttt{caps[name].sigma}
          \item \texttt{mrl.rho\_n\_Co}, \texttt{mrl.rho\_n\_Ti}, default roughness
        \end{itemize}

      \item[materials\_loader.py]
        \texttt{load\_base1\_materials(filename, m\_sld\_from\_x)}  
        returns a \texttt{Materials} dataclass:
        \begin{itemize}
          \item \texttt{substrate: SubstrateSpec}
          \item \texttt{caps: Dict[str, CapSpec]}
          \item \texttt{mrl: MRL} (with \texttt{m\_sld\_from\_x} injected)
        \end{itemize}
    \end{description}
\end{description}

The point is to store all the data in a json file, instead of having it hardcoded anywhere. 

\subsection*{Layer 1}
\begin{description}
  \item[\texttt{physics/reflectometry.py}]
  \begin{description}
    \item[\texttt{\_kz(Q, rho)}] computes complex \( k_z \) in a layer.
    \item[\texttt{parratt\_amplitude(Q, layers, sigmas)}] returns complex reflection amplitude \( \Gamma(Q) \).
    \item[\texttt{reflectivity(Q, layers, sigmas, bkg=0.0)}] returns \( |\Gamma|^2 \) plus background.
    \item[\texttt{spin\_sld(rho\_n, rho\_m, spin)}] returns \( \rho_n \pm \rho_m \) depending on spin.
    \end{description}
\end{description}

This is supposed to be a pure physics engine, meaning that it does not care or now about the optimization, the FoM or the problems. It simply compute $R(Q)$. 

\subsection*{Layer 2 Figure of Merit}
\begin{description}

  \item[\texttt{physics/fom.py}]
  \begin{description}
    \item[\texttt{sensitivity(Q, R\_sub, R\_full)}]
      \( S(Q) = \frac{R_{\text{sub}} - R_{\text{full}}}{R_{\text{sub}} + R_{\text{full}} + \varepsilon} \).

    \item[\texttt{sfm(Q, S)}]
      \( \int |S(Q)| \, \mathrm{d}Q \) for one spin channel.

    \item[\texttt{mcf(Q, S\_up, S\_down)}]
      \( \int |S_{\text{up}}(Q) - S_{\text{down}}(Q)| \, \mathrm{d}Q \).

    \item[\texttt{tsf(foms\_over\_types)}]
      Sum over \( \text{SFM}^{\uparrow} + \text{SFM}^{\downarrow} + \text{MCF}_{\text{diff}} \) for all SOI types.
  \end{description}
\end{description}

This part is also agnostic towrads the search spaces and solvers, it simply computes the FoM. Therefore, reseraching and testing new FoM will be simple. 

\subsection*{Layer 3}
\begin{description}

  \item[\texttt{problems/base1.py}]
  \begin{description}
    \item[Dataclasses:]
      \begin{itemize}
        \item \texttt{SubstrateSpec}, 
        \item  \texttt{CapSpec}, 
        \item \texttt{MRL}, 
        \item \texttt{Materials}
        \item \texttt{SOISpec} 
      \end{itemize}

    \item[\texttt{Base1OptimizationProblem}]
      Implements \texttt{OptimizationProblemProtocol}.
      \begin{itemize}
        \item \texttt{search\_space: SearchSpace}
        \item \texttt{evaluate\_objective(x\_dict) → float (TSF)}
      \end{itemize}

      \textbf{Responsibilities:}
      \begin{itemize}
        \item Builds layer stacks for each SOI and spin using \texttt{Materials} + \texttt{spin\_sld}.
        \item Runs \texttt{reflectivity} for up/down and sub/full.
        \item Computes sensitivities and FOMs via \texttt{fom.sensitivity / sfm / mcf / tsf}.
      \end{itemize}
  \end{description}

  \item[Role in the architecture:]
  This layer forms the bridge from raw physics details to a single optimization target.

  \begin{itemize}
    \item Understands what a design vector is (\( x_{\text{coti}}, d_{\text{mrl}}, \text{cap} \)).
    \item Knows how to evaluate “how good” that design is (via TSF).
  \end{itemize}
\end{description}

\subsection*{Layer 4 Search Space abstraction}
\begin{description}

  \item[\texttt{solvers/search\_space.py}]
  \begin{description}
    \item[\texttt{Param} (abstract)]
      Defines the interface: \texttt{pack}, \texttt{unpack}, \texttt{clip}, \texttt{sample}.

    \item[Concrete params:]
      \begin{itemize}
        \item \texttt{ContinuousParam}
        \item \texttt{IntegerParam}
        \item \texttt{CategoricalParam}
      \end{itemize}

    \item[\texttt{SearchSpace}]
      \begin{itemize}
        \item \texttt{params: List[Param]}
        \item \texttt{pack(x\_dict) return np.ndarray}
        \item \texttt{unpack(vector) return dict}
        \item \texttt{clip(vector) return np.ndarray}
        \item \texttt{sample(n) return np.ndarray} \textit{(once implemented)}
      \end{itemize}
  \end{description}
\end{description}

This is the layer between the actual parameters, and what the optimizer works with. 

\subsection*{Layer 5 Solver}
\begin{description}

  \item[\texttt{solvers/base.py}, \texttt{solvers/single\_obj.py}]
  \begin{description}
    \item[\texttt{Solver} interface (from \texttt{CD.md}):]
      \begin{itemize}
        \item \texttt{run(budget\_evals, time\_limit\_s, callbacks, stopping) → RunResult}
        \item \texttt{ask(n) → List[Dict]} \textit{(or vectors, depending on implementation)}
        \item \texttt{tell(X, Y)}
        \item \texttt{best() → (Dict, float)}
        \item \texttt{reset()}
      \end{itemize}

    \item[Concrete solvers:]
      \begin{itemize}
        \item \texttt{RandomSearchSolver}
        \item \texttt{GridSearchSolver}
        \item (future) CMA, Bayesian, etc.
      \end{itemize}
  \end{description}

  \item[Single-objective behavior:]
  The solver iteratively:
  \begin{enumerate}
    \item proposes candidates via \texttt{ask()},
    \item evaluates them using the objective,
    \item reports results with \texttt{tell()},
    \item tracks best design via \texttt{best()}.
  \end{enumerate}
\end{description}   



\subsection*{Layer 6 UI}
\begin{description}

  \item[\texttt{B1.ipynb}, possibly \texttt{B2.ipynb}]
  \begin{description}
    \item[In these notebooks you:]
      \begin{itemize}
        \item Choose the magnetization model via \texttt{m\_sld\_from\_x}.
        \item Call \texttt{load\_base1\_materials}.
        \item Build the search space and instantiate \texttt{Base1OptimizationProblem}.
        \item Pick a solver and run it.
        \item Plot:
          \begin{itemize}
            \item TSF vs iteration,
            \item reflectivity curves for best design,
            \item sensitivity curves, etc.
          \end{itemize}
      \end{itemize}
  \end{description}

This layer is the interface where one can inspect results, tune parameters, and visualize performance. It is the interactive workflow layer.

\end{description}

\end{document}